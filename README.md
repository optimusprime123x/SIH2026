...
Cloudflare Workers (backend + static assets) · Gemini API (`gemini-3.5-flash-lite`, structured outputs, multi-key rotation) · vanilla JS frontend (Microsoft Metro design language) · zxing-wasm · jsPDF · localStorage repository (D1 + R2 planned — see [TODO.md](TODO.md)).
...
| Path | What |
|---|---|
| [src/worker.js](src/worker.js) | Worker: `/api/scan` (Gemini extraction), `/api/barcode/:gtin` (Open Food Facts), static assets |
...
| [TODO.md](TODO.md) | Roadmap (D1/R2, font-size calibration, e-commerce mode…) |
